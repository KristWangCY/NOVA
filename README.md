...
When no transaction is pending, the block height now stays unchanged. This is normal idle operation, not evidence that the node has stopped: API polling, signed status, peer synchronization, readiness checks, and transaction gossip remain live. The Explorer labels a reachable node with an empty mempool as normally idle. A new transaction is still proposed in the next eligible validator slot.

During a rolling upgrade, a remaining v0.19 validator may still propose a valid empty block. v0.20 validators accept it to preserve one shared history, so occasional empty blocks can continue until every validator has upgraded and restarted. Explorer's normally-idle label describes the connected node's current reachability and empty mempool; it is not a promise that an older peer cannot propose an empty block.
...
- The local Explorer must not be published directly to the internet. Remote access requires a secure, read-only gateway first. v0.20 provides signed remote diagnostics, quorum backups, crash-state cleanup, recovery for common lock states, graceful shutdown, first-use-aware read-only preflight, masked network and CLI launchers, demand-driven block production, conflict-checked Explorer startup, and one-command readiness evidence capture. The owner has completed two verified local readiness days, but not the required seven-day trial or acceptance testing on three physical devices.
...
Architecture decisions are recorded in [ADR 0001](docs/adr/0001-node-prototype.md) through [ADR 0020](docs/adr/0020-demand-driven-block-production.md). See the [product brief](docs/PRODUCT.md), [roadmap](docs/ROADMAP.md), and [protocol summary](docs/PROTOCOL.md) for scope, delivery stages, and protocol formats.
