...
本手册用于准备首次真正的三设备私人网络。v0.19 已提供配置、身份分发、签名远程诊断、quorum 链备份、崩溃状态清理、常见投票锁恢复、优雅停机、首次使用感知的只读 readiness preflight、本地遮罩网络与 CLI 启动器、Explorer 端口隔离和带自动备份的 distributed readiness 采集，但实际三设备私网尚未替你创建。不要跳过“私网与防火墙”步骤，也不要把节点端口映射到公网。
