...
节点只在 mempool 有交易时于下一个合法提议 slot 尝试创建新 proposal。同步可能先提交本地 pending 交易，因此提议者在同步及持久化投票锁恢复之后必须再次检查 mempool；若为空则不创建新区块。API、签名状态、peer 同步和交易传播由独立 HTTP 与同步定时器维持，不依赖空块作为心跳。

v0.20 不改变 block version、交易执行、投票、quorum、状态根或备份格式。历史空块仍是有效的 version 1 committed block，可被完整验证、重放、同步和备份；升级期间，v0.20 节点也会验证并投票给旧节点提出的合法空块，避免协议分叉。已有下一高度的空 proposal 投票锁仍优先恢复。旧配置中的 `emptyBlockIntervalMs` 被安全忽略且不会自动重写；新网络不再生成该字段。所有节点升级并重启后，空闲高度保持不变。
